...
We can easily see the bug that we will bypass the filter

```$filtered = str_replace(['$', '(', ')', '`', '"', "'", "+", ":", "/", "!", "?"], '', $_GET['input']);```

so that it will eval our input

It banned many crucial characters for command injection but i can see the ; is available and we can eval more than two command at all. 
